...
export class NepaliDateConverter {
   ...
   */
  public ADtoBS(englishDate): string {
  ...
  public BStoAD(nepaliDate: string): string {
    ...
    let nepYear = nepaliDateArray[0],
      nepMonth = nepaliDateArray[1],
      nepDay = nepaliDateArray[2];

    //find english date
    let engDate = calendarFunctions.getAdDateByBsDate(
      calendarFunctions.getNumberByNepaliNumber(nepYear),
      calendarFunctions.getNumberByNepaliNumber(nepMonth),
      calendarFunctions.getNumberByNepaliNumber(nepDay)
    );
   ...
   * @param date
   */
  public replaceDate(date: string) {
    if (date != null) {
      if (typeof date === 'object') {
        return date;
      } else {
        return date
          .replace('-', this.dateSplitChar)
          .replace('/', this.dateSplitChar);
      }
    } else return null;
  }
